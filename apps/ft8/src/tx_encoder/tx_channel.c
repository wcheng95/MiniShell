...
/* FT8-only encoder and generator table from MiniFT8-V2 encode.c/constants.c,
 * pinned at 491e757ae6b1e4cfd2b9a6ba10f48b35643849e0.
 * Decoder parity-check tables remain untouched. */
static const uint8_t k_generator[FT8_LDPC_M][FT8_LDPC_K_BYTES] = {
...
{
    uint8_t a91[FT8_LDPC_K_BYTES] = {0};
    memcpy(a91, payload, 10);
    a91[9] &= 0xf8u;
    /* CRC covers the 77 payload bits followed by five zero bits. */
    uint16_t crc = ft8_crc_compute(a91, 82);
    a91[9] |= (uint8_t)(crc >> 11);
    a91[10] = (uint8_t)(crc >> 3);
    a91[11] = (uint8_t)(crc << 5);

    uint8_t codeword[(FT8_LDPC_N + 7) / 8] = {0};
    memcpy(codeword, a91, sizeof(a91));
    for (unsigned row = 0; row < FT8_LDPC_M; ++row) {
        uint8_t parity = 0;
        for (unsigned col = 0; col < FT8_LDPC_K_BYTES; ++col)
            parity ^= a91[col] & k_generator[row][col];
        parity ^= parity >> 4;
        parity ^= parity >> 2;
        parity ^= parity >> 1;
        unsigned bit = FT8_LDPC_K + row;
        codeword[bit / 8] |= (uint8_t)((parity & 1u) << (7u - bit % 8u));
    }

    static const uint8_t costas[7] = {3, 1, 4, 0, 6, 5, 2};
    static const uint8_t gray[8] = {0, 1, 3, 2, 5, 6, 4, 7};
    unsigned bit = 0;
    for (unsigned symbol = 0; symbol < FT8_TX_TONE_COUNT; ++symbol) {
        if (symbol % 36u < 7u) {
            tones[symbol] = costas[symbol % 36u];
        } else {
            unsigned value = 0;
            for (unsigned j = 0; j < 3; ++j, ++bit)
                value = (value << 1) | ((codeword[bit / 8] >> (7u - bit % 8u)) & 1u);
            tones[symbol] = gray[value];
        }
    }
}
