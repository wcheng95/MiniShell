                              ...
                              Js8JsonWrite write, void *context)
{
    ...
    if (metadata->have_utc) {
        char utc[21];
        if (event->elapsed_seconds > UINT64_MAX - metadata->start_seconds ||
            js8_log_format_utc(metadata->start_seconds + event->elapsed_seconds, utc)) {
            return "utc_range";
        }
        field(&line, "utc", utc);
    }
    if (metadata->have_dial)
        append(&line, ",\"dial_hz\":%" PRId64 ",\"rf_millihz\":%" PRId64,
               metadata->dial_hz, metadata->dial_hz*1000 + f->frequency_millihz);
    switch (f->kind) {
    ...
    append(&line, "}\n");
    if (line.failed) return "line_capacity";
    return write(context, line.bytes, line.used) ? "write" : NULL;
}
