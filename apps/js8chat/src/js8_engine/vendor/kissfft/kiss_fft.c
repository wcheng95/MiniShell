                    ...
                    const kiss_fft_cfg st)
{
    ...
    switch (p) {
        ...
        default: return; /* Plan validation permits only factors 2, 3 and 5. */
    }
}
...
kiss_fft_cfg kiss_fft_alloc(int nfft, int inverse_fft, void *mem, size_t *lenmem)
{
    ...
    for (int radix = 2; radix <= 5; ++radix)
        while (factor % radix == 0) factor /= radix;
    if (factor != 1) {
        *lenmem = 0;
        return NULL;
    }
...
{
    /* The monitor always supplies distinct time/frequency storage. */
    if (fin != fout)
