# JS8 Physical Layer Notes

Status: v0.1 PHY reference notes. JS8Call v3.0.3 is the frozen interoperability target.

These notes record the JS8 physical-layer details investigated for the MiniShell built-in **JS8Chat** application. They are a PHY reference, not a standalone project specification.
...
These PHY notes support the frozen MiniShell JS8Chat architecture in `architecture.md`.

For v0.1:

- JS8 Normal is the required speed;
- multi-signal candidate decoding will reuse/adapt the existing Mini-FT8 architecture;
- JS8Call v3.0.3 is the golden interoperability reference;
- exact encoder/decoder behavior must be validated with upstream vectors and WAVs before hardware acceptance.

The DSP implementation should remain MCU-oriented rather than becoming a literal port of desktop JS8Call.
