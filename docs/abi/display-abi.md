...
Status: **implemented on the Linux reference backend; text attributes have begun compatible append-only growth.**
...
Display is output-only. Input is a separate ABI.
...
The current append-only text table contains:
...
`write_at_attr()` was appended after the original text table prefix. A backend that does not implement it may leave the pointer NULL; portable apps must fall back gracefully.
...
Existing text function meanings and offsets are stable within ABI generation 1. New operations are appended. Apps check `struct_size` through the last field they require and also check optional pointers/capabilities.
...
- clear/positioned writes through ANSI below the ABI;
...
The text ABI should remain useful independently of any future graphics sub-API.
