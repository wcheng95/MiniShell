...
- T080 is TESTING: resident `clear` is implemented as a built-in that clears resident console output/history while preserving T075 command history, CWD, aliases/settings and Tab behavior. Linux uses TTY-only ANSI clear/home; ADV resets its retained 50-row console and USB mirror. Software review passed; ADV/pc-1 manual validation remains.
