...
- T033 remains IMPLEMENTING: WebFS browsing/download works, and fresh-boot FT8 failure is now traced to USB Host interrupt-slot exhaustion (`ESP_ERR_NOT_FOUND` from `usb_host_install()`), not heap pressure. Next step is an `esp_intr_dump()` comparison before changing USB/core/interrupt architecture.
...
T033         IMPLEMENTING — fresh-boot USB interrupt exhaustion under investigation
