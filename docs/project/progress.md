...
- T033 returned to IMPLEMENTING after the first hardware test: WebFS SoftAP/browser starts, but password entry is being simplified to 8 lowercase letters and ADV `Filesystem.space()` needs per-volume physical-space fallback so `/flash`/`/sd` listings work without a global quota.
...
T033         IMPLEMENTING — password UX + ADV physical space reporting
