...
- T033 returned to IMPLEMENTING: WebFS browsing/download now works, but current T033 firmware makes FT8 fail even on a fresh boot before WebFS is launched. Retained lwIP state is therefore not the cause; the permanent linked networking SRAM cost is under investigation.
...
T033         IMPLEMENTING — fresh-boot FT8 memory regression under investigation
