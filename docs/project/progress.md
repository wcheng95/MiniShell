...
- T079 is TESTING: when Tab cannot extend an ambiguous pathname prefix, MiniShell now lists all remaining valid pathname choices, then restores the same editable prompt/line/cursor. Linux and ADV share the same two-pass public-Filesystem matcher; ADV listings enter normal 50-row scrollback and preserve T077 cursor behavior. Software review and full automated suites passed; ADV/pc-1 manual validation remains.
