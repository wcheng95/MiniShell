...
- T078 is TESTING: pathname completion is now explicit Tab-only on both ADV and Linux. Typing/paste remain literal; Tab invokes the shared public-Filesystem longest-common-prefix matcher, and no debounce/timer state remains. Software review passed; final ADV and pc-1 manual validation of the Tab interaction remains. T079 is still reserved for showing ambiguous pathname choices when Tab cannot grow the prefix further.
