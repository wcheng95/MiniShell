...
- T033 is READY: ADV WebFS read-only proof. On-demand WPA2 SoftAP + embedded browser UI will browse `/flash` and `/sd` and stream downloads through the existing MiniShell Filesystem API without USB media handoff or cable-role changes.
...
T033         READY — ADV WebFS read-only SoftAP/browser proof
