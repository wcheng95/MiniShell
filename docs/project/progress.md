...
- T082 is COMPLETE: each newly completed MiniFT8 RX batch returns the visible RX screen to page 1, while same-generation redraws preserve operator paging. `cqtypes=` now defines an ordered list of FT8-valid CQ modifiers; plain `CQ` remains first, navigation wraps, the selected numeric `cq_type` persists, and ADV validation passed with `cqtypes=POTA SOTA DX 250` on 2026-09-25.
- T083 is COMPLETE: MiniFT8 QMX CAT/Audio transport faults are contained without terminating the UI loop; the ADV CDC owner task solely owns the CDC handle and blocking driver TX; per-TX `MD6;` was removed; logical RX pause preserves physical UAC continuity. Sustained ADV/QMX normal-operation validation passed on 2026-09-26 with repeated healthy `PREP -> BEGIN -> FIRST_TONE -> END -> RX_RESUME` cycles and no recurrence of the prior UI freeze. A post-T083 blocking audit recorded synchronous logging, diagnostic serialization and bounded-cleanup follow-ups as deferred non-critical performance/diagnostic leads.
