...
static unsigned source_delay_ms;
...
{
    uint64_t samples=UINT64_C(90000000)-9600+input_frames/2+(uint64_t)source_delay_ms*6;
...
static const mini_api_t *probe_api_get(void)
{
    ...
    const char *delay=getenv("MINISHELL_TEST_JS8_DELAY_MS");
    source_delay_ms=delay?(unsigned)strtoul(delay,NULL,10):0;
    return &test_api;
}
