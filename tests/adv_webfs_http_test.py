#!/usr/bin/env python3
"""Execute the production HTTP mutation adapter with a fake request/Filesystem."""
...
fragment = src[src.index('static esp_err_t api_error'):src.index('static esp_err_t handle_request')]
harness = r'''
...
static const mini_fs_api_t fs={.mkdir=mutate,.rmdir=mutate,.remove_file=mutate,.rename=rename_file};
