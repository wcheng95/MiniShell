#!/usr/bin/env python3
"""Directed host diagnostics using fixed independent payload/content fixtures.
...
with tempfile.TemporaryDirectory(prefix='js8-directed-') as temp:
    path = Path(temp) / 'directed.wav'
    for fixture in fixtures:
        tones, payload, diagnostic = fixture.split('\t')
        ...
        assert ' class=directed tx=LAST ' in lines[0], result.stdout
        assert lines[0].endswith(diagnostic), result.stdout
        assert result.stderr.endswith('unique=1\n'), result.stderr
print('js8_directed_wav_test: PASS')
