...
int minishell_app_ft8_main(int argc, char **argv)
{
    ...
    /* Retain shared discovery while the normal UI handles pending CAT. */
    mini_result_t ready = adv_qmx_discovery_begin();
    int result = ready == MINI_OK ? adv_ft8_entry(argc, args) : 12;
    if (adv_qmx_discovery_end() != MINI_OK) result = 12;
    if (result == 12) say_console(mini_api_get(), "ft8: QMX startup/cleanup failed\n");
    return result;
}
