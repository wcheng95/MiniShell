                                     ...
                                     minishell_backend_file_t *out_file)
{
    ...
    *out_file = MINISHELL_BACKEND_FILE_INVALID;
    ...
    int fd;
    do { fd = open(native, oflags, 0666); } while (fd < 0 && errno == EINTR);
                                     ...
                                     void *buffer, uint32_t size, uint32_t *out_read)
{
    ...
    ssize_t count;
    do { count = read(fd, buffer, (size_t)size); } while (count < 0 && errno == EINTR);
                                      ...
                                      uint32_t *out_written)
{
    ...
    ssize_t count;
    do { count = write(fd, buffer, (size_t)size); } while (count < 0 && errno == EINTR);
    if (count < 0) return result_from_errno(errno);
...
static mini_result_t service_fs_sync(void *ctx, minishell_backend_file_t file)
{
    ...
    if (fd < 0) return MINI_ERR_BAD_HANDLE;
    int ret;
    do { ret = fsync(fd); } while (ret != 0 && errno == EINTR);
    return ret == 0 ? MINI_OK : result_from_errno(errno);
}
...
{
    (void)ctx;
    if (out_name == NULL || name_size == 0u || out_type == NULL || out_has_entry == NULL) {
        return MINI_ERR_INVALID;
    }
    out_name[0] = '\0';
    *out_type = 0u;
    *out_has_entry = 0u;
    ...
    for (;;) {
        ...
        if (fd < 0) return result_from_errno(errno);
        if (fstatat(fd, entry->d_name, &st, AT_SYMLINK_NOFOLLOW) != 0) return result_from_errno(errno);
        uint32_t type;
...
static mini_result_t location_path(char *out, size_t out_size)
{
    ...
    return (written < 0 || (size_t)written >= out_size) ? MINI_ERR_NAME_TOO_LONG : MINI_OK;
}
                                                   ...
                                                   int32_t *out_longitude_e7)
{
    ...
    if (matched != 2 || close_result != 0 ||
        ...
        longitude < INT32_MIN || longitude > INT32_MAX) return MINI_ERR_IO;
                                              ...
                                              uint32_t columns)
{
    ...
    for (uint32_t r = 0; r < rows; ++r) {
        if (fprintf(stdout, "\033[%u;%uH", (unsigned)(row + r + 1u),
                    (unsigned)(column + 1u)) < 0) return MINI_ERR_IO;
...
static mini_result_t service_input_wait(void *ctx, uint32_t timeout_ms)
{
    ...
    ssize_t count;
    do { count = read(STDIN_FILENO, buffer, sizeof(buffer)); } while (count < 0 && errno == EINTR);
    if (count < 0) {
